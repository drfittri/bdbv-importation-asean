# PHEIC Importation Risk Estimation — Full Execution Plan

> **Plan v2.** Companion files: `PROGRESS.md` (state tracker) and `CLAUDE.md` (session
> rules). On a new session say **"continue"** → the agent reads PROGRESS.md and resumes.

---

## ⚠️ PLAN v2 — CORRECTIONS & EXECUTION PROTOCOL (AUTHORITATIVE — READ FIRST)

This block overrides any conflicting detail below. The body (Steps 1–38) is retained for
its structure, but the following corrections take precedence wherever they differ.

### A. Identity / data corrections (verified against live sources, June 2026)
1. **Pathogen = Bundibugyo virus (BDBV)**, an *Ebolavirus*. Replace every "IOP" /
   "outbreak pathogen" with "BDBV". CFR is the Bundibugyo range (~25–40%), reported from
   the data — not the invented "25–51%".
2. **INRB repo name is `INRB-UMIE/Ebola_DRC_2026`** (and `BDBV2026-Data`), NOT
   `Ebola_DRC_2026`. Clone the correct one. Dashboard: `inrb-umie.github.io/BDBV2026-Epidemic_Dashboard/`.
3. **Outbreak-region population N = 13,392,200** (Ituri + North Kivu, per ECDC), NOT 5,000,000.
4. **ECDC brief URL slug** = `estimation-importation-risk-bundibugyo-virus-eueea-june-2026`.
   ~1 importation per **24,000** travellers (90% UI 13,000–54,000); 0.45% (0.20–0.85%) for
   100 travellers, 11–25 June. Study window = **11–25 June 2026** (2 weeks).
5. **Delete the hardcoded "837 cases / 196 deaths / 17 June" figure** — unverified.
   Sourced figures: 635 confirmed DRC (9 June), 19 Uganda (11 June); pull latest from the
   INRB repo at run time. Outbreak size (McCabe, *Lancet ID* 2026, DOI
   10.1016/S1473-3099(26)00299-9): ~400–900 cases as of 20 May.

### B. Model approach — REUSE, do not homebrew (overrides Phase 3 entirely)
The old model (`P = 1 − (1 − prevalence × 0.5)^N`) is **retired**. It misrepresents the
ECDC method and a reviewer will reject it. Replacement:
- ECDC's real method is a **6-compartment SEIR-type transmission model** (susceptible /
  exposed-no-symptom / dry-symptom / wet-symptom infectious / recovered / dead), 500 MC
  runs, with **travel probability by state**: susceptible & exposed-no-symptom = baseline;
  dry symptoms = **0.9×**; wet-symptom infectious = **0.1×** (i.e. 90% lower).
- **Step 15 (new):** obtain the released model output (median + draws of exposed and
  infectious counts in the outbreak region) from the ECDC code (PDF reference [9]) or
  `epiforecasts/BVDOutbreakSize`. If neither is runnable, re-implement the ECDC Annex 1
  compartmental model from scratch.
- **Step 16 (new):** importation step =
  `P(≥1 import) = 1 − Π_states (1 − p_travel_state · share_state)^(N · w_state)`,
  i.e. combine state-specific infectious shares with state-specific travel weights and the
  scenario traveller count N. Propagate the model draws → 90% UI.
- **Step 17 (validation):** reproduce ECDC's EU/EEA 0.45% as an **independent cross-check**
  using their stated inputs. Do NOT reverse-tune travel volume to hit the target.

### C. Travel volumes — honest scenarios (overrides Step 13)
DRC/Uganda → ASEAN travel from the *outbreak region* is essentially unmeasurable. Mirror
ECDC: present **hypothetical scenarios of N = 10, 25, 50, 100 travellers** over the 2-week
window, optionally scaled by a transparent connectivity proxy (e.g. ASEAN vs EU/EEA share
of regional outbound seats). Report results per-scenario; never present a single
pseudo-precise point estimate. Flag the exposure caveat: most international travellers
depart Kinshasa (FIH) / Entebbe (EBB), far from the Ituri/North Kivu outbreak zone, so
outbreak-region prevalence overstates the traveller risk — state this in Methods + Limitations.

### D. Rigour additions (do these; they raise publishability)
- Report per a model/observational guideline (STROBE-style for the descriptive part;
  state full model spec + assumptions for the modelling part).
- **Sensitivity analysis** on: ascertainment ratio, traveller count scenario, outbreak
  growth, and state-travel weights. One supplementary table.
- Verify EVERY DOI/URL at `doi.org` before submission. No unverified citations.
- Confirm ethics-waiver wording with your institution's REC (public aggregated data).
- Fix doubled `#PublicHealth` hashtag in Step 36.

### E. Continuity protocol (applies to ALL steps below)
- The agent works one step at a time and, **on completing any step, updates `PROGRESS.md`**
  (flip checkbox, add Completion Log entry with outputs + key numbers + deviations, update
  Current State, append Session Log line). Do not advance past a Quality Gate that fails.
...
Ambiguities found:
  ...
  6. Incubation period and detection delay priors
     Resolution: Use Funk S. 2026 iop-linelist-analysis GitHub (DOI available via
     epiforecasts IOPOutbreakSize repo). This is the same prior used in the Lancet
...
Decision points identified:
  ...
  - Which ECDC model variant to replicate?
    ...
    index-pathogen-eueea-june-2026). It is the most current, peer-reviewed
...
EXECUTION PLAN
══════════════════════════════════════════════════════
Task        : Quantifying the probability of outbreak pathogen (IOP) importation
...
On Fail  : If GitHub push fails, create the repo manually on github.com first
           (New Repository → name: importation-risk-modelling → Public →
...
Output   : README.md exists at ~/importation-risk-modelling/README.md
           containing the following content exactly:

---
# PHEIC Importation Risk Estimation

Reproducible analysis code and data for:

**"Quantifying the probability of outbreak pathogen importation to Malaysia
...
Note     : Do NOT clone inside your project folder. Clone to home directory
           ...
           https://github.com/INRB-UMIE and look for the IOP2026 repo.
...
Output   : CSV file(s) from the epi/ folder exist in
           ~/importation-risk-modelling/data/raw/
           Verify by running: ls ~/importation-risk-modelling/data/raw/
...
Input    : Internet browser. The following three URLs:
           ...
           URL 3: The most recent WHO DON for IOP 2026 (check
                  https://www.who.int/emergencies/disease-outbreak-news
                  and find the latest IOP entry)

Tool     : Web browser → Save Page As → PDF format.
           Save to: ~/importation-risk-modelling/data/raw/
...
Tool     : Create file ~/importation-risk-modelling/data/raw/who_don_series.csv
...
Output   : File ~/importation-risk-modelling/data/raw/who_don_series.csv
...
Input    : URL:
           ...
           importation-risk-index-pathogen-eueea-june-2026

Tool     : Web browser → navigate to URL above → download the PDF linked
           on the page titled "Estimation of the importation risk of
           outbreak pathogen into the EU/EEA in June 2026"
           Save as: ~/importation-risk-modelling/data/raw/ECDC_importation_risk_June2026.pdf
...
Note     : This document is the direct methodological template for your
           ...
           "ECDC. Estimation of the importation risk of outbreak pathogen
           into the EU/EEA in June 2026. Stockholm: ECDC; 2026."

On Fail  : If the ECDC PDF link is broken, access the ECDC outbreak page:
...
URL: https://www.ecdc.europa.eu/en/publications-data/estimation-importation-risk-index-pathogen-eueea-june-2026
DATE ACCESSED: [today's date]
...
Input    : ~/importation-risk-modelling/data/raw/ECDC_importation_risk_June2026.pdf
...
Incubation window used for travel risk calculation: 21 days (maximum,
  per WHO IHR definition for haemorrhagic fever virus family)
...
Input    : File ~/importation-risk-modelling/data/raw/ containing
           CSV files copied from INRB-UMIE in Steps 5-6.
           Also: who_don_series.csv from Step 8 as cross-validation.

Tool     : Create R script ~/importation-risk-modelling/01_data_prep.R
...
Check each item. ALL must be true to proceed.
  ...
  [ ] CFR value in epi_parameters.csv is between 0.05 and 0.55
      (IOP historical CFR range is 25%-51%; if outside this range,
...
Tool     : Create ~/importation-risk-modelling/02_model.R in RStudio.
...
# Incubation period: maximum 21 days (WHO IHR definition for viral haemorrhagic fevers)
# Incubation period: 6-12 days (based on published outbreak data)
incubation_mean <- 9        # days
incubation_sd   <- 3        # days (triangular approximation)
incubation_max  <- 21       # days (WHO IHR)

# IOP CFR range based on published outbreak literature
...
On Fail  : If read_csv throws "file not found" errors, check that
           ...
           the path to ~/importation-risk-modelling/.
──────────────────────────────────────────────────────
```
...
Input    : All files in ~/importation-risk-modelling/
...
Output   : GitHub repository at https://github.com/drfittri/importation-risk-modelling
...
Tool     : Create ~/importation-risk-modelling/03_figures.R with:
...
fig1 <- ggplot(results,
  ...
  coord_flip() +
  labs(
    title    = "Estimated probability of IOP importation to ASEAN countries",
    subtitle = "2-week period, as of June 2026 (90% uncertainty interval)",
    x        = NULL,
    y        = "Probability of at least one importation (%)",
    caption  = paste0(
      "IOP: outbreak pathogen. Monte Carlo simulation (n=10,000 iterations).\n",
...
fig2 <- ggplot(malaysia,
  ...
  labs(
    title    = "IOP importation risk to Malaysia under outbreak growth scenarios",
...
Tool     : RStudio → New File → Markdown File, save as
           ~/importation-risk-modelling/manuscript/manuscript.md
...
Output   : manuscript/manuscript.md exists and contains:
           ...
           TITLE: Must include "outbreak pathogen", "importation",
           "ASEAN" or "Malaysia", and "2026".

           BACKGROUND (40-60 words): State that the outbreak represents a PHEIC,
           CFR range, and that quantified importation risk for Southeast Asia is absent
           from the literature.
...
Input    : manuscript/manuscript.md from Step 23.
           ...
           [3] Feldmann H, Jones S, Schnittler H, Geisbert T. NEJM 2007
               (IOP 2007 Uganda outbreak — CFR ~25%)
...
Output   : Methods section in manuscript.md, 350-450 words, with these
           ...
           available at github.com/drfittri/importation-risk-modelling.
...
Output   : Discussion section in manuscript.md, 350-450 words, with
           ...
           warrants preparedness given elevated CFR.
           ...
           early disease symptoms per ECDC); (c) health care worker
           ...
           ratios from historical outbreak data may not apply to IOP;
...
Output   : Conclusion section ≤100 words stating:
           (1) IOP importation to Malaysia is possible but low-probability
           ...
           [1] WHO. Disease Outbreak News: IOP DRC 2026 (DON602). 2026.
               URL: https://www.who.int/emergencies/disease-outbreak-news/item/2026-DON602
           [2] WHO. Disease Outbreak News: IOP DRC and Uganda (DON603). 2026.
               URL: https://www.who.int/emergencies/disease-outbreak-news/item/2026-DON603
           [3] McCabe R, et al. Estimation of outbreak size in DRC.
               Lancet Infect Dis. 2026. DOI: 10.1016/S1473-3099(26)00299-9
           [4] ECDC. Threat assessment brief: IOP DRC and Uganda. 2026.
               URL: https://www.ecdc.europa.eu/en/...
           [5] ECDC. Estimation of importation risk of outbreak pathogen
           ...
           [7] MOH Malaysia. Kenyataan Media: PHEIC response. May 2026.
...
Tool     : Web browser. Navigate to https://www.medrxiv.org/submit
           ...
           - Data availability: All data and code at
             github.com/drfittri/importation-risk-modelling
...
Tool     : Web browser. Navigate to:
           ...
           - Keywords: outbreak pathogen, importation risk,
...
The ongoing outbreak of the pathogen in the Democratic Republic
...
the probability of IOP importation to Malaysia, Singapore, Thailand, and
...
## Citation
[Your name]. Quantifying the probability of outbreak pathogen importation
...
Tool     : LinkedIn (web or app). Write a post of 150-200 words with
           ...
           (f) Tags: #OutbreakModelling #PublicHealth #PublicHealth #Epidemiology
...
Note     : Tag the CEPI LinkedIn page directly if your account allows.
           ...
           officers who monitor the IOP outbreak literature.
...
Tool     : Microsoft Word. Create manuscript/policy_brief.docx
           ...
           TITLE: PHEIC Importation Risk to Malaysia — Key Estimates
           DATE: [today]
...
Note     : Send via email to your MAE contacts with the subject line:
           "PHEIC Importation Risk Estimates for Malaysia — Policy Brief"
...
Run these checks after Step 38. All must pass.
  ...
  [ ] GitHub repo https://github.com/drfittri/importation-risk-modelling
...
*Pathogen: outbreak pathogen (IOP), PHEIC context*
*Target: Eurosurveillance Rapid Communication (primary) / WPSAR (secondary)*
*Repository: github.com/drfittri/importation-risk-modelling*
